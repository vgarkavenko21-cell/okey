...
async def handle_premium_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "premium_subscribe":
        ...
        try:
            await query.edit_message_text(
                "🔗 **Premium-канали**\n\n"
                "Щоб отримати Premium на 7 днів, потрібно підписатися на всі канали.\n\n"
                "Після того, як ви це зробите, натисніть **«✅ Перевірити підписку»**.\n"
                "Якщо бот виявить, що ви підписані — вам буде видано Premium.\n"
                "У разі відписки від каналу Premium буде анульовано.",
                reply_markup=InlineKeyboardMarkup(keyboard),
                parse_mode="Markdown",
            )
        except BadRequest as e:
